...
class SLoraLayer(BaseTunerLayer):
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            # layer output
            sub_batch = x[sub_batch_indices_list[i]].to(lora_S.dtype)
            if self.spectral_type[active_adapter]=='exp':
                lora_output = lora_B(lora_S.exp() * lora_A(dropout(sub_batch))) * scaling
            elif self.spectral_type[active_adapter]=='relu':
                lora_output = lora_B(F.relu(lora_S) * lora_A(dropout(sub_batch))) * scaling
...
class Embedding(nn.Module, SLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        SLoraLayer.__init__(self, base_layer)
...
class Conv2d(nn.Module, SLoraLayer):
