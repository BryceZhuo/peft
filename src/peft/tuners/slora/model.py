...
from .layer import Conv2d, SLoraLayer, dispatch_default
...
class SLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: SLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: SLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            "spectral_type": lora_config.spectral_type,
        ...
        if isinstance(target, SLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            if model_config["model_type"] not in TRANSFORMERS_MODELS_TO_SLORA_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = set(
                TRANSFORMERS_MODELS_TO_SLORA_TARGET_MODULES_MAPPING[model_config["model_type"]]
            )
        return peft_config
